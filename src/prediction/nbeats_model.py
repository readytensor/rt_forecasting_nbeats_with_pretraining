...
class NBeatsNet:
    ...
    def has_exog(self): return self.exo_dim > 0
    ...
    def fit(self, *args, **kwargs):        
        return self.n_beats.fit(*args, **kwargs)

    
    def predict(self, *args, **kwargs):        
        return self.n_beats.predict(*args, **kwargs)
    ...
    def create_block(self, x, e, stack_id, block_id, stack_type, nb_poly):
        ...
        def reg(layer): return self._r(layer, stack_id)
...
if __name__ == '__main__':
    ...
    model = NBeatsNet(
        input_dim=2,            # D = 2
        exo_dim=0,
        backcast_length=10,     # Tx = 10
        forecast_length=5,      # Ty = 5
    ...
    model.compile_model(loss='mse', learning_rate=1e-2)
    model.summary()


    inputs = np.random.randn(50,10,2)  # is of shape (N, Tx, D) => (50, 10, 2)
    preds = model.predict(inputs)
    print(preds.shape)
    # prints (50, 5, 2)             # is of shape (N, Ty, D) => (50, 5, 2) 
